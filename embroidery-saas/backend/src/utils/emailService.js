...
exports.sendPasswordResetEmail = async (email, token, name) => {
    ...
    const mailOptions = {
        from: `"Core Logic SaaS" <${process.env.EMAIL_USER}>`,
        to: email,
        subject: 'Reset Your Password - Core Logic Embroidery SaaS',
        html: `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 12px; background-color: #ffffff;">
                <div style="text-align: center; margin-bottom: 30px;">
                    <h1 style="color: #7c3aed; margin: 0; font-size: 28px;">Core Logic</h1>
                    <p style="color: #6b7280; font-size: 14px; text-transform: uppercase; letter-spacing: 2px; font-weight: bold; margin-top: 5px;">Secure Access</p>
                </div>
                
                <div style="padding: 20px; color: #374151; line-height: 1.6;">
                    <h2 style="font-size: 20px; color: #111827;">Password Reset Request</h2>
                    <p>Hello ${name}, we received a request to reset your password for your Core Logic account. Click the button below to set a new password:</p>
                    
                    <div style="text-align: center; margin: 40px 0;">
                        <a href="${resetUrl}" style="background: linear-gradient(135deg, #ef4444 0%, #b91c1c 100%); color: #ffffff; padding: 14px 30px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px; box-shadow: 0 4px 6px -1px rgba(239, 68, 68, 0.2);">Reset Password</a>
                    </div>
                    
                    <p style="font-size: 14px; color: #6b7280;">This link is valid for 1 hour. If you didn't request a password reset, you can safely ignore this email.</p>
                </div>
                
                <div style="border-top: 1px solid #f3f4f6; margin-top: 30px; padding-top: 20px; text-align: center;">
                    <p style="font-size: 12px; color: #9ca3af; margin: 0;">This email was sent to ${email}.</p>
                    <p style="font-size: 12px; color: #9ca3af; margin-top: 5px;">&copy; 2026 Core Logic Inc. All rights reserved.</p>
                </div>
            </div>
        `
